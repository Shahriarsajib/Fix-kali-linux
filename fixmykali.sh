...
# fix_xfce_root fix_xfce_user fix_xfcepower external configuration file
    raw_xfce="https://drive.google.com/file/d/1UrLauXSEfgJsL6lJZZwG1Qc2Jj-j1g3X/view?usp=sharing"
...
check_for_root () {
    if [ "$EUID" -ne 0 ]
      then echo -e "\n\n Script must be run with sudo ./fixmykali.sh or as root \n"
      exit
    else
...
# add google-chrome due to gowitness dependancy
check_chrome(){
...
# Wont work if qterminal is the active terminal running fixmykali - find another way
...
# disable_power_xfce replaces fix_xfce_power fix_xfce_user and fix_xfce_root functions
disable_power_xfce() {
    if [ $finduser = "root" ]
     then
...
#  Current version of spike throws undefined symbol error, revert to old version
fix_spike () {
    echo -e "\n  $greenplus Fix SPIKE "
...
fix_set() {
    # move these to their individual respecitive functions at a later date
...
fix_go_path() {
    ...
    if [ $check_for_displayzero == 1 ]
     ...
     # with a kali user on tty7 (:0) and then kali pts/1, as fixmykali.sh is being run with sudo and was producing this fault
...
# fix_sources updated to add wildcards
fix_sources () {
    fix_bad_apt_hash
    # relaxed grep
...
run_update () {
    fix_sources
    echo -e "\n  $greenplus starting: fixmykaliupgrade   \n"
    apt_update && apt_update_complete
...
# new screens for copying from /home/kali to /root, no detection, all based on used input
ask_homekali_to_root () {
...
fix_sead_warning () {
 ...
 "$bold$redexclaim$red" WARNING "$redexclaim$bold$red"  FIXMYKALI IMPACKET REMOVAL FUNCTION  "$redexclaim$bold$red" WARNING "$redexclaim$white$norm"
   ...
   This function of fixmykali is reserved for the most severe cases of broken
...
#---- begin fixmykali-mirrors ----
get_mirrorlist() {
  	cleanup
    fix_sources
    echo -e "\n  $greenplus Fixmykali-Mirrors - kali repo mirror speedtest"
...
#---- end fixmykali-mirrors  ----

# ascii art - DONT move
asciiart=$(base64 -d <<< "")
...
#    echo -e "\n  Menu Options:"                                                                    # function call list
    ...
    echo -e "  9 - FixmyKaliupgrade         (apt upgrade with vbox/vmware detection)"               # only_upgrade
    echo -e "                               (sources.list, linux-headers, vm-video)"                # -
    echo -e "  0 - Fix ONLY 1 thru 8        (runs only 1 thru 8) \n"                                # fix_all
    echo -e "  "$bold"N - NEW VM SETUP"$reset" - Run this option if this is the first time running fixmykali\n"
    echo -e "  = - Fixmykali-Mirrors       (find fastest kali mirror. use the equals symbol = )"   # get_mirrorlist; best_ping; small_speedtest; large_speedtest; gen_new_sources; cleanup;;
    ...
    case $menuinput in
      ...
      x|X) echo -e "\n\n Exiting fixmykali.sh - Happy Hacking! \n" ;;
      *) fixmykali_menu ;;
    esac
    }
